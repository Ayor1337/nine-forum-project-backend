...
- [x] 本地用户端 `application.yml` 存在、包含所需本机配置且被 Git 忽略，不出现在索引或待提交文件中。
- [x] `application.example.yml` 可解析、结构完整且不含真实 SMTP 凭据。
- [x] 新克隆可按 README 从示例复制出本地配置。
- [x] 重写后的所有目标分支与标签均不包含用户端 `application.yml` 历史路径。
- [x] 已验证重写后的仓库可以正常检出，并提供重新克隆/强制同步说明及回滚点。
- [x] 不修改本地 Docker 凭据、JWT 共享登录设计或无关审计报告改动。
